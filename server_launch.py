...
place that knows the server is invoked as `python -m mcp_server.entrypoint` is
...
def server_command(database_path: Path) -> tuple[str, list[str], str]:
    ...
    return sys.executable, ["-m", "mcp_server.entrypoint", "--database", str(database_path)], str(ROOT)
